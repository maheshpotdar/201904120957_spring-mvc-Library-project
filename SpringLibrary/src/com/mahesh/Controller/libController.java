...
public class libController {
	...
	public String getRegister(@Valid @ModelAttribute("beanRegister") BeanRegister beanRegister, ModelMap modelMap) {
		modelMap.addAttribute("beanRegister", new BeanRegister(0, false, "", "", "", ""));
		return "register";
	}
	...
	public String getadminDashboard(@Valid @ModelAttribute("beanRegister") BeanRegister beanRegister,
		...
		if (bindingResult.hasErrors()) {
			...
			if (dbcon.isValidUser(beanRegister)) {// register ahe
				if (dbcon.isaprove(beanRegister) == "admin") {
					return "adminDashboard";
				} else if (dbcon.isaprove(beanRegister) == "lib") {
					return "libSection";
				} else {
					return "noaprv";
				}
	...
	@RequestMapping("/noaprv")
	public String getnoaprv() {
		return "noaprv";
	}

	@RequestMapping("/viewlib")
	public String getviewLib(@ModelAttribute("beanRegister") BeanRegister beanRegister, ModelMap modelMap) {
	...
	public String getdeleteLib(@ModelAttribute("beanRegister") BeanRegister beanRegister, ModelMap modelMap,
	...
	public String geteditLib(@ModelAttribute("beanRegister") BeanRegister beanRegister, ModelMap modelMap,
			@PathVariable("id") int id) {
		// baherun id alela ahe so forfully setter and access there variable.
		BeanRegister beanRegister2 = dbcon.getEditlib(id);
		beanRegister2.setId(id);
		modelMap.addAttribute("beanRegister2", beanRegister2);
		return "editlibs";
	}
	...
	/*
	 * @RequestMapping(value = "/updtlib/{userId}",method = RequestMethod.POST)
	 * public void UpdateEmpFormAction(@ModelAttribute("beanRegister") BeanRegister
	 * beanRegister, ModelMap modelMap,
	 * 
	 * @PathVariable("userId") int userId) { System.out.println("first" +
	 * beanRegister); int val = dbcon.updateLib(beanRegister, userId);
	 * System.out.println("second");
	 * 
	 * if (val > 0) return "redirect:/viewlib"; else return "redirect:/failed";
	 * 
	 * }
	 */
	@RequestMapping(value = "/updtlib/{userId}", method = RequestMethod.POST)
	public String UpdateEmpFormAction(@PathVariable("userId") int userId,
			...
			@ModelAttribute("beanRegister2") BeanRegister beanRegister2, ModelMap modelMap) {

		int val = dbcon.updateLib(beanRegister2, userId);
		if (val > 0)
			return "redirect:/viewlib";
		else
			return "redirect:/failed";
	 ...
	 * public String UpdateEmpFormAction(@PathVariable("userId") int userId,
	 * 
	 * @ModelAttribute("beanRegister2") BeanRegister beanRegister2, ModelMap
	 * modelMap) { System.out.println("mahesh");
	 * System.out.println("updatetttttttt 217: " + beanRegister2); int val =
	 * dbcon.updateLib(beanRegister2, userId); if (val > 0) return
	 * "redirect:/viewlib"; else return "redirect:/failed"; }
	 */
